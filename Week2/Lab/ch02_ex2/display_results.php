<?php
    ...
    // validate years entry
            // validate interest rate entry
    if ( empty($years) ) {
        $error_message .= '<p class="error"> Years is a required field. </p>'; }
    else if ( !is_numeric($interest_rate) )  {
        $error_message .= '<p class="error"> Years must be a valid number. </p>'; }
    else if ( $years <= 0 || $years > 50) {
        $error_message .= '<p class="error"> Years must be greater than zero or less than or equal to 50. </p>'; }
